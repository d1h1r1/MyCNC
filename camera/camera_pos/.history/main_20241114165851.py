...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 使用自定义的ROILabel替换self.label
        self.label = ROILabel(self)
        # self.label.setGeometry(50, 50, 640, 480)
        self.label.roi_selected.connect(self.on_roi_selected)  # 连接ROI选择信号
    ...
    def take_photo(self):
        ...
        if ids is not None and len(ids) >= 2:  # 至少需要检测到两个标记
            # 绘制检测到的标记
            print(ids)
            aruco.drawDetectedMarkers(self.last_frame, corners, ids)
            ...
            if len(ids) >= 2:
                ...
                # 裁剪图像
                cropped_frame = self.last_frame[y_min:y_max, x_min:x_max]
    ...
    def on_roi_selected(self, roi_rect):
        ...
        print(f"ROI位置和大小：X={x}, Y={y-180}, Width={w}, Height={h}")
        print(f"矩形左上角坐标：({x}, {y-180})")
        print(f"矩形左下角坐标：({x}, {y-180+h})")
        print(f"矩形右下角坐标：({x+w}, {y+h-180})")
        print(f"矩形右上角坐标：({x+w}, {y-180})")
        print(f"矩形中心点坐标：({x+w//2}, {y+h//2-180})")

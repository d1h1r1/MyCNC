...
def main(stl_path="test1.stl"):
    ...
    upward_mesh = keep_vertical_upward_faces(stl_path)

    upward_mesh = filter_occluded_upward_faces(
        full_mesh=full_mesh,
        upward_mesh=upward_mesh
    )

    if upward_mesh is None:
        print("错误: 没有找到垂直向上的面！")
        return
    ...
    # 2. 按高度聚类面
    # print("\n2. 按高度聚类面...")
    clusters = cluster_faces_by_height(upward_mesh, height_tolerance=0.01)
    if not clusters:
        print("错误: 没有找到任何平面！")
        return
    ...
    # 3. 提取每个平面的轮廓
    # print("\n3. 提取平面轮廓...")
    planes = extract_plane_contours(upward_mesh, clusters)
...
if __name__ == '__main__':
    # 指定你的STL文件路径
    stl_file = "球顶全遮挡.stl"  # 修改为你的文件路径
    # stl_file = "test1.stl"  # 修改为你的文件路径
    main(stl_file)

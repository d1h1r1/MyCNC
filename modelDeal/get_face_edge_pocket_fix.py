...
def get_points11(mesh, clusters):
    for i, cluster in enumerate(clusters):
        print(f"平面 {i+1}")
...
def extract_plane_contours(mesh, clusters):
    ...
    for i, cluster in enumerate(clusters):
        ...
        edges = extract_boundary_edges(mesh, used_vertex_indices)
        points = edges_to_loops(edges)
        print(points)
        # print(f"  实际顶点坐标:")
